...
import YoutubeEmbed from '../YoutubeVedio';
...
const ExploreBrands = () => {
    ...
    const getYoutubeLink = async () => {
        const data = await getDocs(collection(db, "youtubeLink"));
        setYoutubeLink(data.docs.map((doc) => ({ ...doc.data(), id: doc.id })));
    };
    useEffect(() => {
        getYoutubeLink();
    }, [])
        ...
        <>
            <div style={{ margin: '20px 70px 0px 70px' }}>
                <div style={{ display: "flex", flexDirection: 'row', width: '100%', flexWrap: 'wrap', justifyContent: 'center', gap: 20 }}>
                    {youtubeLink && youtubeLink.map((e, i) => {
                        return (
                            <ReactPlayer controls url={e.link} width={340} height={240} />
                        )
                    })}
                </div>
            </div>
