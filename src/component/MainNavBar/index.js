...
const StyledInputBase = styled(InputBase)(({ theme }) => ({
  ...
  "& .MuiInputBase-input": {
    ...
    paddingLeft: `calc(.5em + ${theme.spacing()})`,
...
export default function MainNavBar({ setItemSearch, cartCount }) {
      ...
      >
        <List>
          ...
          >
            ...
            >
              <Typography sx={{ color: "#000000", fontSize: "20px", fontFamily: "Gill Sans" }}>
                Home
              </Typography>
            </Link>
          </ListItemText>
  ...
  const getMarqueeText = async () => {
    const data = await getDocs(collection(db, "marquee"));
    setMarqueeText(data.docs.map((doc) => ({ ...doc.data(), id: doc.id })));
  };
  useEffect(() => {
    getMarqueeText();
  }, [])
  ...
  return (
    <Box className={classes.navheight}>
      <AppBar color="default" className={classes.root} position="fixed">
        ...
        {marqueeText && marqueeText.map((e, i) => {
          return (
            <marquee
              width="100%"
              direction="right"
        ...
        <Container className="container">
          ...
          >
            ...
            >
              <SearchIcon sx={{ fontSize: "20px", fontFamily: "Gill Sans", }} />
            </IconButton>
          </Search>
        </Container>
        {/* start part */}
